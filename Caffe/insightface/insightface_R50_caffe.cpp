...
int main()
{
	...
	caffe_net_test = new caffe::Net<float>("./model/R50-0000.prototxt", caffe::TEST);

	caffe::NetParameter trained_net_param_test;

	if (!caffe::ReadProtoFromBinaryFile("./model/R50-0000.caffemodel", &trained_net_param_test)) {
    ...
    cv::resize(src_img, src_img, cv::Size(600, 600));
