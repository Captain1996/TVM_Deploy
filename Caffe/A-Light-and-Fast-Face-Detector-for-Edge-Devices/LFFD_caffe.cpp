...
int main()
{
	...
	caffe_net_test = new caffe::Net<float>("/caffemodel/configuration_10_320_20L_5scales_v2/symbol_10_320_20L_5scales_v2_deploy.prototxt", caffe::TEST);

	caffe::NetParameter trained_net_param_test;

	if (!caffe::ReadProtoFromBinaryFile("/caffemodel/configuration_10_320_20L_5scales_v2/train_10_320_20L_5scales_v2_iter_1000000.caffemodel", &trained_net_param_test)) {
	...
	cv::Mat src_img = cv::imread("2.jpg"); //refer to /Ultra-Light-Fast-Generic-Face-Detector-1MB/imgs/2.jpg
	...
	for(int c=0; c<rchannels; ++c)
		for(unsigned int i=0; i<volChl; ++i)
		{
			pixel = static_cast<float>((float(src_img.data[i*3 + c]) - 127.5)/ 127.5);
    ...
    float prob_0 = caffe_net_test->blob_by_name("conv20_3_bbox")->data_at(0,0,0,0);
	...
	for(int i=0; i<23; ++i)
	{
		...
		prob_0 = caffe_net_test->blob_by_name("conv20_3_bbox")->data_at(0,0,0,0);
		if(i == 2)
		{
