...
int main()
{
	...
	caffe_net_test = new caffe::Net<float>("/model/mobilenet_deploy.prototxt", caffe::TEST);

	caffe::NetParameter trained_net_param_test;

	if (!caffe::ReadProtoFromBinaryFile("/model/mobilenet.caffemodel", &trained_net_param_test)) {
