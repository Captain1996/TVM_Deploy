...
void Mat2CHW(float *data, cv::Mat &frame, const int height, const int width, const int channel = 3)
{
	...
	for(int c=0; c<channel; ++c)
		...
		{
			if(c == 0)
				data[c*volChl + i] = static_cast<float>((float(frame.data[i*3 + c]) - 103.94));
			if(c == 1)
				data[c*volChl + i] = static_cast<float>((float(frame.data[i*3 + c]) - 116.78));
			if(c == 2)
				data[c*volChl + i] = static_cast<float>((float(frame.data[i*3 + c]) - 123.68));
		}
}
...
int main()
{
    ...
    tvm::runtime::Module mod_syslib = tvm::runtime::Module::LoadFromFile("../lib/vgg16_deploy_lib.so");
    printf("load lib.so ok.\n");

    // json graph
    std::ifstream json_in("vgg16_deploy_graph.json", std::ios::in);
    std::string json_data((std::istreambuf_iterator<char>(json_in)), std::istreambuf_iterator<char>());
    json_in.close();
    printf("load graph ok.\n");

    // parameters in binary
    std::ifstream params_in("vgg16_deploy_param.params", std::ios::binary);
