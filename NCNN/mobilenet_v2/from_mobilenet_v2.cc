...
int main()
{
    ...
    tvm::runtime::Module mod_syslib = tvm::runtime::Module::LoadFromFile("../lib/mobilenet_v2_deploy_lib.so");
    printf("load lib.so ok.\n");

    // json graph
    std::ifstream json_in("mobilenet_v2_deploy_graph.json", std::ios::in);
    std::string json_data((std::istreambuf_iterator<char>(json_in)), std::istreambuf_iterator<char>());
    json_in.close();
    printf("load graph ok.\n");

    // parameters in binary
    std::ifstream params_in("mobile_v2_deploy_param.params", std::ios::binary);
