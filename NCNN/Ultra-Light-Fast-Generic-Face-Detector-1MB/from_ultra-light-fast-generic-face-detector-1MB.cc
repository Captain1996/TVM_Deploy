...
void Mat2CHW(float *data, cv::Mat &frame, const int height, const int width, const int channel = 3)
{
	...
	for(int c=0; c<channel; ++c)
		...
		{
			data[c*volChl + i] = static_cast<float>((float(frame.data[i*3 + c]) - 127.0)/ 128.0);
		}
}
...
int main()
{
    ...
    tvm::runtime::Module mod_syslib = tvm::runtime::Module::LoadFromFile("../lib/ultra-light-fast-generic-face-detector-1MB_deploy_lib.so");
    printf("load lib.so ok.\n");

    // json graph
    std::ifstream json_in("ultra-light-fast-generic-face-detector-1MB_deploy_graph.json", std::ios::in);
    std::string json_data((std::istreambuf_iterator<char>(json_in)), std::istreambuf_iterator<char>());
    json_in.close();
    printf("load graph ok.\n");

    // parameters in binary
    std::ifstream params_in("ultra-light-fast-generic-face-detector-1MB_deploy_param.params", std::ios::binary);
    ...
    DLTensor* x;
    int in_ndim = 4;
    int64_t in_shape[4] = {1, 3, 240, 320 };
    ...
    cv::resize(frame, input, cv::Size(320, 240));
    float data[320 * 240 * 3];
    Mat2CHW(data, input, 320, 240, 3);

    memcpy(x->data, &data, 3 * 320 * 240 * sizeof(float));
    printf("load data ok.\n");
